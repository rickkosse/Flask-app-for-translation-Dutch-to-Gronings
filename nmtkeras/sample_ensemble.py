...
def parse_args_char(direction):
    if direction == "NL_GRO":
        ...
        parser.add_argument("-ds", "--dataset", required=False,
                            default=os.getcwd() + "/nmtkeras/datasets/char/Dataset_EuTrans_nlgro.pkl",
                            help="Dataset instance with data")
        parser.add_argument("-t", "--text", required=False, default=os.getcwd() + "/Output.txt",
        ...
        parser.add_argument("-c", "--config", required=False, default=os.getcwd() + "/nmtkeras/char_config.pkl",
        ...
        parser.add_argument("-g", "--glossary", required=False, help="Glossary file for overwriting translations.")
        parser.add_argument("-m", "--models", nargs="+", required=False, default=[
            os.getcwd() + "/nmtkeras/trained_models_char/EuTrans_gronl_AttentionRNNEncoderDecoder_src_emb_32_bidir_True_enc_LSTM_32_dec_ConditionalLSTM_32_deepout_linear_trg_emb_32_Adam_0.001/epoch_22"],
                            help="Path to the models")
        parser.add_argument("-ch", "--changes", nargs="*", help="Changes to the config. Following the syntax Key=Value",
                            default="")
        return parser.parse_args()
...
def parse_args_bpe(direction):
    if direction == "NL_GRO":
        parser = argparse.ArgumentParser(
            "Use several translation models for obtaining predictions from a source text file.")
        parser.add_argument("-ds", "--dataset", required=False,
                            default=os.getcwd() + "/nmtkeras/datasets/bpe/Dataset_EuTrans_nlgro.pkl",
                            help="Dataset instance with data")
        parser.add_argument("-t", "--text", required=False, default=os.getcwd() + "/output_bpe_encoded.txt",
                            help="Text file with source sentences")
        parser.add_argument("-s", "--splits", nargs='+', required=False, default=['val'], help="Splits to sample. "
                                                                                               "Should be already included"
                                                                                               "into the dataset object.")
        parser.add_argument("-d", "--dest", default="./output_bpe_encoded_translated.txt",
                            help="File to save translations in. If not specified, "
                                 "translations are outputted in STDOUT.")
        parser.add_argument("-v", "--verbose", required=False, default=0, type=int, help="Verbosity level")
        parser.add_argument("-c", "--config", required=False, default=os.getcwd() + "/nmtkeras/bpe_config.pkl",
                            help="Config pkl for loading the model configuration. "
                                 "If not specified, hyperparameters "
                                 "are read from config.py")
        parser.add_argument("-n", "--n-best", action="store_true", default=False,
        ...
        parser = argparse.ArgumentParser(
            "Use several translation models for obtaining predictions from a source text file.")
        parser.add_argument("-ds", "--dataset", required=False,
                            default=os.getcwd() + "/nmtkeras/datasets/bpe/Dataset_EuTrans_gronl.pkl",
                            help="Dataset instance with data")
        parser.add_argument("-t", "--text", required=False,
        ...
        parser.add_argument("-g", "--glossary", required=False, help="Glossary file for overwriting translations.")
        parser.add_argument("-m", "--models", nargs="+", required=False, default=[
            os.getcwd() + "/nmtkeras/trained_models_bpe/EuTrans_gronl_AttentionRNNEncoderDecoder_src_emb_32_bidir_True_enc_LSTM_32_dec_ConditionalLSTM_32_deepout_linear_trg_emb_32_Adam_0.001/epoch_17"],
                            help="Path to the models")
        parser.add_argument("-ch", "--changes", nargs="*", help="Changes to the config. Following the syntax Key=Value",
                            default="")
        return parser.parse_args()

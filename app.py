...
def init():
    ...
    #
    char_args, char_params, char_models, char_dataset = sample_ensemble.load_in("char")
    # bpe_args, bpe_params, bpe_models, bpe_dataset = sample_ensemble.load_in("BPE")
...
def char_get_predictions():
    ...
    encoding= "char"
    pred = sample_ensemble.predict(char_args, char_params, char_models, char_dataset)
    return pred
...
def predict_predict_nl_gro():
    ...
    if request.method == 'POST':
        ...
        print("voor restoring", output)
        output_sen = restore(output)
        print("vertaling is ",output_sen)
